...
import { X, Download, Copy, Share2, Upload, Trash2, ZoomIn } from 'lucide-react';
...
const ClubTournamentResultsDialog = ({ open, onClose, tournament }) => {
    ...
    // Form State
    const [sharingMode, setSharingMode] = useState('overall');
    ...
    const players = tournament?.result ? getPlayersFromResult(tournament.result) : [];
    ...
    const generateImage = async () => {
        const node = document.getElementById('club-tournament-poster');
        if (!node) return;
    ...
    const handleDownload = async () => {
        ...
        if (dataUrl) {
            ...
            const venueName = tournament?.venue?.name || 'tournament';
            link.download = `${venueName}_${tournament?.category || 'results'}.png`;
            link.href = dataUrl;
            link.click();
        ...
        if (dataUrl && navigator.share) {
            try {
                ...
                await navigator.share({
                    files: [file],
                    title: `${tournament?.category} Results`,
                    text: `Check out the results of ${tournament?.category} at ${tournament?.venue?.name}!`,
                });
            } catch (err) {
                        ...
                        {sharingMode === 'own' && (
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 3 }}>
                                ...
                                <Box className="avatar-upload-container">
                                    ...
                                    />
                                    <label htmlFor="club-avatar-upload">
                                        <Button
                                            component="span"
                                            variant="outlined"
                                            fullWidth
...
export default ClubTournamentResultsDialog;
